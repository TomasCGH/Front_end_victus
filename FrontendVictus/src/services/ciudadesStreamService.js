...
const STREAM_URL = `${API.streamV1}/ciudades/stream`; // Prefijo streams real
...
export function subscribeToCiudadesStream({ onCreated, onUpdated, onDeleted, onError, onOpen } = {}) {
  ...
  const normalizePayload = (data) => ({
    ...data,
    payload: data?.payload ?? data?.data ?? data?.ciudad ?? data,
  });
  ...
  const handleNominal = (tipo) => (event) => {
    ...
    try {
      ...
      const normalizedPayload = parsed?.payload ?? parsed?.data ?? parsed?.ciudad ?? parsed;
